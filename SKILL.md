...
The default workflow is detail-first whenever a carpet pair is generated. It is especially important for local raster designs and exact detail-to-overview matching because the macro detail carries the strongest construction evidence and can inform the lower-information overview.
...
2. Generate and inspect `material_detail` first from the current design source, the deterministic `design_detail_crop`, and the selected construction references. Treat the crop as a hard local pattern lock: preserve its motif lines, relative positions, boundaries, adjacency, negative space and colour placement, allowing only fibre-scale softening. The design source and crop own the visible local motif, boundary and colour placement; construction references own only the physical weave. Use an attractive professional three-quarter macro: camera approximately 25–35 degrees above the rug plane, azimuth 20–35 degrees across the two adjoining edges, with the corner and one bound edge forming a diagonal leading line into depth. Keep the near corner junction complete, both binding segments visible, near and central interlacing sharp, and the far field gently softened. Avoid both a flat straight-down shot and an extreme near-zero grazing shot. Do not use a generated overview as a pattern or detail reference because it does not exist yet and cannot provide macro-quality evidence.
3. Accept the detail as `material_detail_anchor` only when its selected corner, local motif topology, camera, yarn hierarchy, relief, boundary behaviour and colour placement pass. The detail anchor owns texture identity, interlacing, relief, yarn direction, finish and local physical appearance only; it never overrides the design source's motifs or colours. If the detail fails, do not let it control the overview; record the failed categories and use a targeted detail retry or report the mismatch.
4. Generate `product_overview` only after an accepted `material_detail_anchor` exists. Use the current design source as the sole authority for the complete outline, motif count, placement, adjacency, symmetry, negative space and final colours. Use the accepted detail anchor as the primary texture reference for yarn character, relief, interlacing, boundary behaviour and light response, with the selected product-scale construction reference supplying only scale and edge evidence. Match a professional woven-rug sample photograph from a standing observer: use a restrained near-overhead downward angle, slightly closer framing, and only mild natural near-to-far perspective so the near lower edge is just slightly larger than the far upper edge. Keep the rug flat and square to the frame, the long axis vertical, the short edges horizontal, and at most 0–2 degrees of residual rotation. Show the complete bound outline and four corners, filling about 92–96% of the frame with narrow, quiet pale floor margins and a small soft contact shadow. Keep the perspective calm: no diagonal camera roll, dramatic foreshortening, keystone distortion, or wide-angle distortion. Never feed the overview back into detail generation.
...
Read [prompt templates](references/prompt-templates.md) for the selected path, then use the selected backend in edit/reference mode. Generate both outputs without waiting for user approval.
...
For a preview-only backend outside `standard_reference_jacquard`, the following legacy fallback still uses the same detail-first order when a deterministic design crop is available. Use no more than three supporting references in addition to the design source: normally macro detail, product-scale crop, and overall edge evidence only when needed. Do not infer or attach images from earlier conversation turns.
...
3. Generate the overview from the current design, accepted `material_detail_anchor`, and only the scale, binding, paired-mapping, or style references needed. Use the accepted detail as the primary texture reference: preserve its yarn family, relief, interlacing, direction and finish while reducing the apparent size to product distance. Use the accepted standing-observer, restrained near-overhead sample-shot view with the rug deliberately squared to the image frame, long edges nearly vertical, short edges nearly horizontal, and only mild near-to-far perspective. Match the canvas to the rug aspect ratio where possible; keep the complete bound outline visible while filling about 92–96% of the frame with narrow, quiet floor margins. Preserve the upright design orientation so `detail_corner` maps to the same visible corner. A product-scale reference may refine density and edge evidence, but it must not replace the accepted detail's construction identity or control motifs and colours.
...
For a controllable local backend, keep topology and material on separate branches: region/edge controls from the current design own structure, a reference or IP-Adapter branch owns material, and a mask owns local repair. For strict cross-scale consistency, first generate and approve the macro detail from the deterministic design crop, then generate the overview while reusing that accepted detail anchor on the material branch. Preserve the seed and all independent control values. Do not derive the detail from the overview in the default workflow.
