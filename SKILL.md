...
When `render_branch: wilton_flatweave` is selected, keep the current design as the sole pattern and colour authority. First compare the source SHA-256 with declared paired mappings. On an exact pair match, attach the complete current design first, its matching physical product overview second, and its matching material detail third; omit generic quality and micro anchors. Otherwise, attach the complete design, real camera anchor, and multi-unit micro construction anchor in that order. A quality anchor is optional and may control only exposure, white balance, and general product-photo finish; it must never control yarn geometry or camera geometry. The micro anchor validates local yarn-unit geometry, exposed cross yarn, and gap proportion; the grammar tile remains audit evidence for lane staggering and open channels. No secondary reference can provide current motifs, layout, or colours.
...
Read [capabilities](CAPABILITIES.md), [backend strategies](references/backend-strategies.md), and [exact topology lock](references/topology-lock.md) before choosing a rendering backend. Use one of two explicit routes. The default for a user-facing Wilton visual sample is the **reference-grounded visual route**: use the exact-paired packet when the source SHA matches a declared pair; otherwise attach complete design, declared camera-angle anchor, and multi-unit micro construction anchor. Record `topology_mode: approximate`. The **exact-proof route** creates only an aligned structure proof and anchor report; it is not a texture renderer. Reference-image input alone never upgrades an approximate visual to exact. Record the actual backend, reference paths and roles, topology mode, proof path, and `surface_build_mode` in the render lock.
...
For the reference-grounded visual route, read [the Wilton flatweave branch](references/wilton-flatweave-branch.md). On an exact paired match attach current design, matching paired overview, and matching paired material detail. Otherwise attach current design, declared camera-angle anchor, and declared multi-unit micro construction anchor. Label the result `topology_mode: approximate`. Do not generate a product overview until the user accepts the material detail. `task_reference_grounded` is allowed only when the recorded generation request actually attached the real construction image; otherwise use `library_grounded` or `profile_only`.
...
4. For the user-facing visual route, first resolve an exact paired mapping. If one matches the source hash, attach complete current `design_source`, matching paired product overview, and matching paired material detail, in that order, and omit all generic anchors. Otherwise attach the complete current design, declared camera-and-relief anchor, and multi-unit micro construction anchor. The complete design owns all outline, binding, inner lock line, motif, spacing, and colour relationships. Real references control only physical rendering and camera; the optional quality anchor controls only exposure, white balance, and overall product-photo finish. Record every actual attachment and omitted conflicting anchor in the lock. Do not substitute the local proof renderer for this step.
...
6. Only after accepted detail review, generate `product_overview` from the full current design and the same exact-paired mapping (when available), or the declared real construction reference otherwise. Keep the same construction family, bundle direction, relief, colour mapping, edge treatment, and light direction. Label the overview `topology_mode: approximate`; it is a visual sample, not proof of exact topology.
