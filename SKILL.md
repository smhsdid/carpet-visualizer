...
5. For both outputs use one directional raking key light at a shallow side angle plus soft fill. Require visible micro-shadow in recessed channels, clear separation between raised crowns and lower ground, controlled warm soft-satin highlights on taupe and ivory yarns, and no plastic glare, washed-out ivory, grey veil or global pale binder grid. Relief must come from woven over-under interlacing and compression, not inflated beads or independent cords. Compare the selected corner in both outputs against `design_detail_crop`; any local redraw, missing diamond, merged line, shifted boundary or wrong colour is a topology failure.
...
For a preview-only backend outside `standard_reference_jacquard`, the following is a legacy fallback only when a deterministic design crop or overview-first workflow is unavailable. Use no more than three supporting references in addition to the design source: normally neutral macro detail, neutral product-scale crop, and overall edge evidence only when needed. Do not infer or attach images from earlier conversation turns.
