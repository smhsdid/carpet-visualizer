...
For a preview-only backend, use no more than three supporting references in addition to the design source: normally neutral macro detail, neutral product-scale crop, and overall edge evidence only when needed. Generate in this order:

1. Generate a close contextual material detail of `detail_corner`. Use an ultra-close macro product photograph from a grazing oblique viewpoint, with the camera roughly 8–15 degrees above the rug plane and aimed diagonally across the surface. Place the lens nearly level with and just above the near binding so its side face is prominent, nearby yarns appear distinctly larger than distant yarns, and the surface strongly recedes through foreshortening. Let the rug surface occupy about 94–98% of the frame. Retain the complete near corner junction, very short segments of both adjoining edges, and only enough nearby border artwork to locate the crop. Use grazing soft light and sufficient depth of field to keep the near and central interlacing legible. A one-time style reference may control camera distance, angle, crop, focus falloff and lighting only; construction remains owned by the selected construction evidence.
2. Inspect the detail before using it as an anchor. Camera passes only when the near binding side face is prominent and the near-to-far scale change, surface recession and focus falloff match a genuinely low grazing macro view. For `jacquard-01`, mentally rectify perspective into upright rug coordinates: vertical raised crowns remain dominant but expose a continuous horizontal fine-yarn rhythm through plain areas, motifs and colour boundaries. The vertical crowns use two related scales, with larger dominant crowns and smaller secondary crowns naturally interleaved. As a visual exposure target, coarse vertical crowns occupy about 55–65% of the woven surface, clearly exposed fine horizontal yarns about 25–35%, and recessed vertical ground threads the remaining channels. Relief passes only when soft twisted strands bend over, compress against and sink between the finer yarn systems with small natural variation. Use the detail as `material_detail_anchor` only when the corner, camera, direction, yarn balance and relief all pass. Otherwise omit it from overview references, record the failed categories, and report the failure.
