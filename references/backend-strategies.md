...
For `wilton-flatweave-01` details, attach the complete current design first, a generic real detail camera-and-construction anchor from a different paired design second, and the multi-unit micro construction anchor third. The overview and detail from a pair that matches the current design hash are forbidden generation references. The design alone controls outline, binding, inner lock line, motifs, colours, and every spacing relationship. The camera anchor controls focal relationship and framing; the micro anchor controls local yarn geometry. A quality anchor is optional finish evidence only and may not control geometry. Record all actual attachments.
