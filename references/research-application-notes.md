...
本项目默认采用 `detail_first`：先用画稿和确定性 `design_detail_crop` 生成并检查高质量 material detail，再把通过检查的细节图作为肌理、纱线层级、交织、起伏和光泽锚点生成 product overview。这样先解决最难的局部肌理，同时保持画稿对全貌轮廓、花纹、区域关系和颜色的绝对控制。只有用户明确要求或后端强制先锁定产品结构时，才允许例外顺序；即使例外，也不能让全貌图反向定义细节肌理。
